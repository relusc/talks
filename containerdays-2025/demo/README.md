...
- `policy-controller`: includes a Sigstore Policy Controller `ClusterImagePolicy` to verify the signature of images
...
Just execute the `./setup/setup.sh` script. This will start colima, create the `kind` cluster, setup the Sigstore Policy Controller and initiate a login to `ghcr.io`.
...
- verify images using the `policy-controller` policy
